...
SEC_FLAGS=(--shm-size "$SHM_SIZE" -p "$BIND_ADDR:$PORT:$PORT" --label "$MANAGED_LABEL=1")
...
if [ -n "${DRY_RUN:-}" ]; then
  ...
  echo "[dry-run] absent     --privileged --network=host --ipc=host (runtime socket not mounted)"
